...
# Find second largest value in binary search tree.
...
def find_second_largest(root_node):
    if root_node == None or (root_node.left is None and root_node.right is None):
    ...
    while current:
        ...
        if current.left and not current.right:
            return find_largest(current.left).value

        if current.right and not (current.right.left or current.right.right):
